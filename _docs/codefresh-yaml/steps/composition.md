...
>Note: In order to mount the shared volume on one of your composition services, you must mount it on the `composition_candidate` also. It is not compulsory to mount the shared volume in all services of a composition. Only those that actually use it for file transfer, should mount it.
